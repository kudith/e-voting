...
const handleDeleteElection = async () => {
  if (!selectedElection || !selectedElection.id) {
    console.error("No election selected or invalid election ID.");
    toast.error("Failed to delete election. Invalid election selected.");
    return;
  }

  try {
    setIsDeleting(true);
    setDeletingElectionId(selectedElection.id);
              ...
              <ElectionTable
                ...
                onDelete={(election) => {
                  if (!election || !election.id) {
                    console.error("Invalid election selected for deletion.");
                    toast.error("Invalid election selected.");
                    return;
                  }
                  setSelectedElection(election);
